...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[url('/images/bg-main.svg')] bg-cover pb-12">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 rounded-3xl p-8 md:p-12 text-white border border-indigo-800 shadow-xl relative overflow-hidden mb-10">
          ...
          <div className="relative z-10 max-w-3xl space-y-5">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-[10px] font-bold uppercase tracking-wider">
        ...
        {/* ── Empty State ── */}
        {!loadingResumes && resumes.length === 0 && (
          <div className="bg-white border border-gray-200 rounded-3xl p-12 text-center shadow-sm max-w-md mx-auto mt-6">
            <div className="w-16 h-16 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-5 text-2xl">
              📂
            </div>
            <h3 className="text-base font-bold text-gray-800 mb-2">No analyses yet</h3>
            <p className="text-xs text-gray-500 max-w-xs mx-auto mb-6 leading-relaxed">
              Upload your resume and the target job description to get a detailed rating and career pathway analysis.
            </p>
            <Link
              to="/upload"
              className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-5 py-2.5 rounded-xl transition text-xs shadow-sm hover:shadow"
            >
              Upload Resume
            </Link>
          </div>
      ...
      {/* ── Toast Notifications ── */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4.5 py-3 rounded-2xl shadow-xl border backdrop-blur-md transition-all duration-300 animate-in slide-in-from-bottom-5 ${
          toast.type === "success" 
